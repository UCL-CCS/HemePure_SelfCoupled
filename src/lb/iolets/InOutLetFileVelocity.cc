	...
	namespace lb
	{
		namespace iolets
		{
			...
			void InOutLetFileVelocity::Initialise(const util::UnitConverter* unitConverter)
			{
				...
				coupledMaxVelocity = false;
#ifdef HEMELB_USE_VELOCITY_WEIGHTS_FILE
				useWeightsFromFile = true;

				if (!util::file_exists(velocityFilePath.c_str())){
					coupledMaxVelocity = true;
				}
				...
				if (useWeightsFromFile) {
					...
					weightAve = weightAve/weights_table.size();
				}
			}

                        //JM  From parabolic
			LatticeForceVector InOutLetFileVelocity::GetForceOnBoundary() const
 		        {
 			      return latticeForce;
			}
