	...
	namespace lb
	{
		namespace iolets
		{
			InOutLetParabolicVelocity::InOutLetParabolicVelocity() :
				maxSpeed(0.), warmUpLength(0), densityMean(1.0)
			{
			}
					...
					const LatticeTimeStep t) const
			{
				...
				double temp;
				//Dimensionless rSq = (displ.GetMagnitudeSquared() - z * z) / (radius * radius); //JM original layout
				Dimensionless rSq = (displ.GetMagnitudeSquared() - z * z) / ((radius - modf(radius/sqrt(2),&temp)*sqrt(2)) * (radius - modf(radius/sqrt(2),&temp)*sqrt(2))); //JM test for boundary error
